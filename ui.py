...
class UI:
    ...
    def main_window(self):
        ...
        self.words_count_label = tk.Label(self.header_frame, text=f"Words inputted: {words_learned}/{goal}")
        ...
        self.new_button = tk.Button(self.buttons_frame, text="New", width=7,
                                    command=lambda: self.activate_button('new'))
        self.practice_button = tk.Button(self.buttons_frame, text="Practice", width=7,
                                         command=lambda: self.activate_button('practice'))
        self.quiz_button = tk.Button(self.buttons_frame, text="Quiz", width=7,
                                     command=lambda: self.activate_button('quiz'))
        self.settings_button = tk.Button(self.buttons_frame, text="Settings", width=7,
                                         command=lambda: self.activate_button('settings'))
    ...
    def activate_button(self, button_name):
        ...
        for button in [self.new_button, self.practice_button, self.quiz_button, self.settings_button]:
            button.config(relief=tk.RAISED, state=tk.NORMAL)

        # Set the clicked button to the active state
        if button_name == 'new':
            self.new_word()
            self.active_button = self.new_button
        elif button_name == 'practice':
            self.practice()
            self.active_button = self.practice_button
        elif button_name == 'quiz':
            self.quiz()
            self.active_button = self.quiz_button
        elif button_name == 'settings':
            self.settings()
            self.active_button = self.settings_button

        if self.active_button:
            self.active_button.config(relief=tk.SUNKEN, state=tk.DISABLED)
    ...
    def update_words_count_label(self):
        words_learned, goal = self.svc.get_progress()
        self.words_count_label.config(text=f"Words inputted: {words_learned}/{goal}")
...
class Practice:
    ...
    def _load_random_meaning(self, event=None):
        ...
        if words:
            ...
            self.meaning_label.config(text=f"{selected_word.meaning}; {selected_word.pos}", anchor='w', justify=tk.LEFT)
...
class Quiz:
    ...
    def show(self):
        ...
        # Question label
        question_label = tk.Label(self.frame, text="What is the word for:", justify="left")
        question_label.grid(row=1, column=0, pady=2, sticky="w", columnspan=2)

        # question_label.grid(row=1, column=0, pady=2, sticky="we")
        self.meaning_label = tk.Label(self.frame, text="", wraplength=270, justify="left")
        self.meaning_label.grid(row=2, column=0, columnspan=2, sticky="we")

        # Answer buttons
        self.buttons = []
        for i in range(3):
            choice_button = tk.Button(self.frame, command=lambda b=i: self.check_answer(b), justify="center")
            choice_button.grid(row=3 + i, column=0, columnspan=2, pady=2,  sticky="nsew")
